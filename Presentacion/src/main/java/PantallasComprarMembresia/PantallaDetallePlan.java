...
public class PantallaDetallePlan extends PantallaBase {
    ...
    private JPanel crearPanelExtras() {
        ...
        // Obtener nombres de amenidades que el plan ya incluye para filtrarlas
        PlanDTO planActual = controlador.getPlanSeleccionado();
        List<String> nombresIncluidos = new ArrayList<>();
        if (planActual != null && planActual.getAmenidades() != null) {
            for (AmenidadDTO a : planActual.getAmenidades()) {
                nombresIncluidos.add(a.getNombre().toLowerCase().trim());
            }
        }
        ...
        // Filtrar los extras que ya vienen incluidos en el plan
        List<AmenidadDTO> extrasFiltrados = new ArrayList<>();
        if (extras != null) {
            for (AmenidadDTO extra : extras) {
                if (!nombresIncluidos.contains(extra.getNombre().toLowerCase().trim())) {
                    extrasFiltrados.add(extra);
                }
            }
        }

        if (extrasFiltrados.isEmpty()) {
            JLabel sinExtras = new JLabel("No hay extras adicionales para este plan.");
            sinExtras.setFont(Colores.FUENTE_LABEL);
            sinExtras.setForeground(Colores.TEXTO_PLACEHOLDER);
            card.add(sinExtras);
        } else {
            for (AmenidadDTO extra : extrasFiltrados) {
