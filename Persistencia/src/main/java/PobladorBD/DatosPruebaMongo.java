...
public class DatosPruebaMongo {
    ...
    public static void main(String[] args) {
     ...
     public static void poblarBD() {
         ...
         * Limpiar colecciones
         */
        bd.getCollection("amenidades").deleteMany(new Document());
        ...
        MongoCollection<Document> colAmenidades =
                bd.getCollection("amenidades");

        colAmenidades.insertOne(
                AmenidadPersistenciaMapper
                        .toDocument(pesas));
         ...
         * =========================
         */
        SucursalPojo sucursal1 =
                new SucursalPojo();

        sucursal1.setIdSucursal("S001");
        sucursal1.setNombre("Centro");
        ...
        sucursal1.setAmenidadesSucursal(
                List.of(pesas, alberca));

        SucursalPojo sucursal2 =
                new SucursalPojo();
        ...
        sucursal2.setAmenidadesSucursal(
                List.of(pesas));

        MongoCollection<Document> colSucursales =
                bd.getCollection("sucursales");

        colSucursales.insertOne(
                SucursalPersistenciaMapper
                        .toDocument(sucursal1));

        colSucursales.insertOne(
                SucursalPersistenciaMapper
                        .toDocument(sucursal2));
        ...
        entrenador.setHorarios(
                List.of(horario));

        MongoCollection<Document> colEntrenadores =
                bd.getCollection("entrenadores");

        colEntrenadores.insertOne(
                EntrenadorPersistenciaMapper
                        .toDocument(entrenador));
         ...
         * =========================
         */
        MembresiaPojo membresia =
                new MembresiaPojo();

        membresia.setIdMembresia("M001");
        membresia.setIdCliente("CL001");

        membresia.setIdPlan("P001");
        membresia.setIdSucursal("S001");

        membresia.setAmenidadesExtra(
                List.of(alberca));
        ...
        membresia.setFechaTramite(
                LocalDateTime.now());

        membresia.setFechaCaducidad(
                LocalDateTime.now()
                        .plusMonths(1));

        membresia.setEstado(
                EstadoMembresiaPojo.ACTIVA);

        String fechaVigencia =
                membresia.getFechaCaducidad()
                        .format(
                                DateTimeFormatter.ofPattern(
                                        "yyyy-MM-dd"));

        String urlQR =
                "https://steelcorefitness.com/acceso"
                + "?id=" + membresia.getIdMembresia()
                + "&cliente=" + membresia.getIdCliente()
                + "&plan=" + membresia.getIdPlan()
                + "&sucursal=" + membresia.getIdSucursal()
        ...
        pago.setEstado(
                EstadoPagoPojo.COMPLETADO);

        pago.setFecha(LocalDateTime.now());

        membresia.setPago(pago);

        MongoCollection<Document> colMembresias =
                bd.getCollection("membresias");

        colMembresias.insertOne(
                MembresiaPersistenciaMapper
                        .toDocument(membresia));
        ...
        cita.setEstado(
                EstadoCitaPojo.CONFIRMADA);

        cita.setNotas(
                "Primera cita de bienvenida");
        ...
        usuario.setRol(
                RolUsuarioPojo.CLIENTE);

        MembresiaActivaPojo snapshot =
                new MembresiaActivaPojo();

        snapshot.setIdMembresia("M001");
        snapshot.setIdPlan("P001");

        snapshot.setFechaCaducidad(
                membresia.getFechaCaducidad());
        ...
        cliente.setMembresiaActiva(
                snapshot);

        //cliente.setCitaBienvenida(cita);

        MongoCollection<Document> colClientes =
                bd.getCollection("clientes");

        colClientes.insertOne(
                ClientePersistenciaMapper
                        .toDocument(cliente));
        ...
        MongoCollection<Document> colVisitas =
                bd.getCollection("visitas");

        colVisitas.insertOne(
                VisitaPersistenciaMapper
                        .toDocument(visita));
