...
public class ControlComprarMembresia {
    ...
    public List<AmenidadDTO> obtenerAmenidadesDePlan(String idPlan) {
        ...
        if (plan == null || plan.getAmenidades() == null) return new ArrayList<>();
     ...
     * (campo codigoQR), con el esquema "steelcore://acceso/{idMembresia}".
     * El empleado escanea este QR con la misma app de escritorio para
     * validar el acceso del cliente.
     *
     * @param idMembresia identificador de la membresía a codificar
     * @return bytes PNG listos para ser convertidos en ImageIcon, o null si falla
     */
    ...
    public MembresiaDTO obtenerMembresiaActiva(String idCliente) {
        for (MembresiaDTO m : membresiaBO.obtenerPorCliente(idCliente))
            if (m.getEstado() == EstadoMembresia.ACTIVA) return m;
        return null;
    }
